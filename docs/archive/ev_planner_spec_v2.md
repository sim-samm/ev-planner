...
Moves are the unit of cost because they are easy to track in-game.
...
### Combined Metric: EV per Move

```python
ev_per_move = (ev_from_battle + ev_from_money) / moves_per_ko
```

This ratio guides whether one battle option is better than another.
...
See [sub_problems.md](sub_problems.md) for the full breakdown, interfaces, and dependency graph.
...
- **Time-based metric:** Supplement EV-per-move with a real-time estimate (encounter rate, animation time, step count).
